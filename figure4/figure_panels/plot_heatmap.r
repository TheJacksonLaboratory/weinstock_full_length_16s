...
df <- read.table('isolate_error_profiles.tsv', row.names=1, sep='\t', header=TRUE)
...
isolate.tree.dend.col <- color_branches(isolate.tree.dend.col, 
                                        k=length(labels(isolate.tree.dend.col)),
                                        , col=leaf.cols)
