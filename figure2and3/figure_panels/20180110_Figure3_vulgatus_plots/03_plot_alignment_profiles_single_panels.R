...
plt <- ggplot(df.a, aes(x=BasePosition, y=bacteroides_vulgatus)) + geom_line()
plt <- plt + theme_bw()
...
plt <- plt + geom_hline(yintercept=seq(-100,100, by=100/8),
...
pdf('03_bvulgatus_atcc8482_only_panel.pdf', height=3, width=5.5)
plot(plt)
dev.off()
...
plt <- ggplot(df.b, aes(x=BasePosition, y=bacteroides_vulgatus)) + geom_line()
plt <- plt + theme_bw()
plt <- plt + theme(panel.grid = element_blank())
...
pdf('03_bvulgatus_mpk_only_panel.pdf', height=3, width=5.5)
plot(plt)
dev.off()
