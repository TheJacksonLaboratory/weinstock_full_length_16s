##################################################################################################
#
# Plot predicted E. coli MG1655 vs Sakai
#
##################################################################################################
library('ggplot2')

## Load the substitution data matrices... 
df <- read.table('04_mg1655_vs_sakai_counts.tsv', header=T, sep='\t')
...
df$Position[df$Panel=="Sakai"] <- df$Position[df$Panel=="Sakai"] - 1

head(df)

table(df$Panel)
df.c <- df[df$Panel=='MG1655',]
df.d <- df[df$Panel=='Sakai',]
df.d$Substitutions <- df.d$Substitutions*-1

###################################################################################################
## Plot the panel C figure
plt <- ggplot(df.c, aes(x=Position, y=Substitutions)) + geom_line()
plt <- plt + theme_bw()
...
plt <- plt + geom_hline(yintercept=seq(-1,1, by=1/7),
...
pdf('02C_THEOR_ECOLI.pdf', height=3, width=3.5)
plot(plt)
dev.off()

###################################################################################################
## Plot the panel D figure
plt <- ggplot(df.d, aes(x=Position, y=Substitutions)) + geom_line()
plt <- plt + theme_bw()
plt <- plt + theme(panel.grid = element_blank())
plt <- plt + ylim(c(0, 1.2))
...
pdf('02D_THEOR_ECOLI.pdf', height=3, width=3.5)
plot(plt)
dev.off()
